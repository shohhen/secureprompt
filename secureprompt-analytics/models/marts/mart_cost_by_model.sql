{{ config(
    materialized='table',
    engine='MergeTree()',
    order_by='(model, usage_date)',
    partition_by='toYYYYMM(usage_date)'
) }}

-- mart_cost_by_model: cumulative cost per model with 7-day and 30-day rolling windows.
-- Used by /usage cost breakdown panels in the dashboard.
--
