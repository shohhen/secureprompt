...
/// Every column `RequestEventRow` writes — taken FROM that struct, not
/// transcribed from it.
///
/// MR1 review I8: this used to be a hand-maintained array of 24 string
/// literals with nothing tying it to `RequestEventRow`'s serialised fields.
/// Deleting an entry compiled, passed every test, and left the startup probe
/// permanently blind to that column — i.e. blind to exactly the "worker's
/// ClickHouse migrations have not run, every analytics event is being dropped
/// wholesale" condition the probe exists to detect. The probe's own test could
/// not see it either: the fixture builds its "complete" table FROM this
/// constant, so `missing` was empty by construction.
///
/// `<RequestEventRow as Row>::COLUMN_NAMES` is generated by the `Row` derive
/// from the struct's fields, in declaration order, so the list cannot drift
/// from what the writer serialises. Adding a field to `RequestEventRow` now
/// widens the probe automatically; the remaining — and separate — hazard is
/// adding a field without a matching `clickhouse/migrations/*` ALTER, which is
/// what `probe_expectation_matches_the_migrated_schema` in
/// `tests/clickhouse_schema_probe.rs` measures against the live database.
pub const REQUEST_EVENTS_COLUMNS: &[&str] = <RequestEventRow as Row>::COLUMN_NAMES;
